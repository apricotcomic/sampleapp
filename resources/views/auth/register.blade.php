...
<form name="registform" action="/auth/register" method="post">
    ...
    パスワード：<input type="text" name="password" size="30"><span>{{ $errors->first('password') }}</span><br />
    パスワード（確認）：<input type="text" name="password_confirmation" size="30"><span>{{ $errors->first('password_confirmation') }}</span><br />
